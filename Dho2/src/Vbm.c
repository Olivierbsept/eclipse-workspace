...
void FumToVbmMsg(uint32_t mn, uint32_t mh);
void RamToVbmMsg(uint32_t mn, uint32_t mh);
extern VbmToFumMsg(uint32_t mn, uint32_t mh);
extern VbmToRamMsg(uint32_t mn, uint32_t mh);
Mnh getMnh_Vbm(void);
...
void Vbm_step10ms(){
	...
	else if (mn0==mn1 && mn0==mn2 && mn0!=0xFFFFFFFF && RxTooSlow1==false && RxTooSlow2==false){
		mh0= max3(mh0, mh1, mh2);
		// modification des mh1 et mh2
		TxSlow1 = false;
		TxSlow2 = false;
	}
	else if (mn0==mn1 && mn0!=0xFFFFFFFF && RxTooSlow1==false){
		mh0= max2(mh0, mh1);
		// modification de mh1 et pas du mh2
		TxSlow1 = false;
		TxSlow2 = true;
	}
	else if (mn0==mn2 && mn0!=0xFFFFFFFF && RxTooSlow2==false){
		mh0= max2(mh0, mh2);
...
void TxHighCmd(bool enable){
	TxFastForSync=enable;
	printf("FreqHighOn : %d\n", TxFastForSync);
}
