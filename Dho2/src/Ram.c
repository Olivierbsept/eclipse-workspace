...
void VbmToRamMsg(uint32_t mn, uint32_t mh);
void FumToRamMsg(uint32_t mn, uint32_t mh);
extern void RamToVbmMsg(uint32_t mn, uint32_t mh);
extern void RamToFumMsg(uint32_t mn, uint32_t mh);
Mnh getMnh_Ram(void);
static uint32_t mn0=101;
static uint32_t mh0=500;
...
void Ram_step10ms(){
	compteurStep++;
	if (	(true == TxSlow1 && 0 == (compteurStep % 150))
		||	(false == TxSlow1 && false == TxFastForSync && 0 == (compteurStep % 100))
		||	(true == TxFastForSync && 0 == (compteurStep % 10)) ){
		RamToVbmMsg(mn0,mh0);
	}
	if (	(true == TxSlow2 && 0 == (compteurStep % 150))
			||	(false == TxSlow2 && false == TxFastForSync && 0 == (compteurStep % 100))
			||	(true == TxFastForSync && 0 == (compteurStep % 10)) ){
		RamToFumMsg(mn0,mh0);
	}
